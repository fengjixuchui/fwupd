...
fu_vli_usbhub_pd_device_probe (FuDevice *device, GError **error)
{
	...
	instance_id = g_strdup_printf ("VLI_USBHUB_PD\\VID_%04X&PID_%04X",
...
fu_vli_usbhub_pd_device_read_firmware (FuDevice *device, GError **error)
{
	...
	fw = fu_vli_usbhub_device_dump_firmware (parent,
					...
					GError **error)
{
	...
	if (!fu_vli_usbhub_device_erase_sectors (parent,
						 fu_vli_usbhub_pd_chip_get_offset (self->chip),
						 bufsz,
						 error))
		return FALSE;
	...
	if (!fu_vli_usbhub_device_write_blocks (parent,
						fu_vli_usbhub_pd_chip_get_offset (self->chip),
						buf,
						bufsz,
						error))
